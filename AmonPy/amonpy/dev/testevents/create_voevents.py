...
for kk in xrange(nevents):
    ...
    if (events[kk].stream == 0):
        fname='/yourpath/events_icecube/event_%s.xml' % (kk,)
    elif (events[kk].stream == 1): 
        fname='/yourpath/events_antares/event_%s.xml' % (kk,) 
    elif (events[kk].stream == 3): 
        fname='/yourpath/events_auger/event_%s.xml' % (kk,) 
    elif (events[kk].stream == 7): 
        fname='/yourpath/events_hawc/event_%s.xml' % (kk,)
    else:
