...
from amonpy.dbase import ofualert_to_voevent
...
def ic_ofu_config():
    ...
    """
    stream = alert_streams['OFU-Alerts']
    rev = 0
    config = AlertConfig2(stream,rev)
    config.participating  = alert_streams['OFU-Alerts']#2**streams['IC-HESE'] + 2**streams['IC-EHE'] # index of event streams
...
def ic_ofu(new_event=None):

    max_id = db_read.alert_max_id(alert_streams['OFU-Alerts'],HostFancyName,
                                       UserFancyName,
                                       PasswordFancy,
                                       DBFancyName)
    if max_id is None:
