...
[tags IceCube-%s]""" % (notice_type, event_datetime, str(event.stream), str(run_id),
    str(event_id), event.RA, event.dec, src_error_90*60., src_error_50*60., str(event.rev), energy/1000.,
...
[tags HAWC-%s]""" % ("HAWC Burst Monitor", str(event.datetime), str(event.stream), str(run_id), str(event_id),
    event.RA, event.dec, event.sigmaR*60., str(event.rev), event.false_pos, event.pvalue, str(event.deltaT), tag) 
