...
def ehealert_to_voevent(alert, params, duplicate):
    stream=2
    amon_id = alert[0].id
    rev=alert[0].rev
    dupl=duplicate
    ...
    for i in range(len(params)):
        ...
        if (params[i].name== 'energy'):
            energy=params[i].value/1000 # convert to TeV from GeV
    ...
    v.set_ivorn("ivo://amon/icecube_ehe#%s" % str(stream)+'_'+str(run_id)+'_'+str(event_id)+'_'+ str(rev))
    v.set_role("%s" % alert[0].type)
    if (alert[0].type=='observation'):
        if (dupl==False):
            v.set_Description("Report of IceCube EHE neutrino alert.")
            same_as_hese = 0
        else:
            v.set_Description("Report of IceCube EHE neutrino alert. This alert was generated by the same neutrino event as the previous alert(s) with the same event and run numbers.")
            same_as_hese = 1
    else:
        if (dupl==False):
            v.set_Description("Report of IceCube EHE test neutrino alert.")
            same_as_hese = 0
        else:
            v.set_Description("Report of IceCube EHE test neutrino alert. This alert was generated by the same neutrino event as the previous alert(s) with the same event and run numbers.")
            same_as_hese = 1
    w = Who()
    ...
    p.set_Description(["IceCube EHE stream identification"])
    w.add_Param(p)
    ...
    p.set_Description(["Energy estimate - lower bound (in TeV)."])
    w.add_Param(p)

    p = Param(name="same_as_hese", ucd="meta.number", unit=" ", dataType="int",  value=str(same_as_hese))
    p.set_Description(["Value of 1 means this alert is based on the same neutrino as HESE alert with the same run and event numbers."])
    w.add_Param(p)
