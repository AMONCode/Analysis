...
def ICcascade_to_voevent(alert, params, skymaps):
    ...
    elif stream==streams['IC-Cascade']:
        v.set_ivorn("ivo://amon/icecube_cascade#%s" % str(stream)+'_'+str(run_id)+'_'+str(event_id)+'_'+ str(rev))
        v.set_Description("Report of IceCube Cascade neutrino event.")
    ...
    if stream==streams['IC-Gold']:
        ...
        p.set_Description(["IceCube Cascade stream identification"])
    w.add_Param(p)
    ...
    p = Param(name="src_error_50", ucd="stat.error.sys", unit="deg", dataType="float",  value=str(src_error_50))
    p.set_Description(["Angular error of the source (50% containment)"])
    w.add_Param(p)
    ...
    pos2=Position2D("deg-deg", "RA","Dec",value2, 0) # TODO
