...
def hesealert_to_voevent(alert, params,duplicate):
    stream=1
    id = alert[0].id
    rev=alert[0].rev
    dupl=duplicate
    ...
    if (alert[0].type=='observation'):
        if (dupl==False):
            ...
            v.set_Description("Report of IceCube HESE neutrino alert. This alert was generated by the same neutrino event as the previous alert(s) with the same run and event numbers.")
            same_as_ehe = 1
    else:
        if (dupl==False):
            v.set_Description("Report of IceCube HESE test neutrino alert")
            same_as_ehe = 0
        else:
            v.set_Description("Report of IceCube HESE test neutrino alert. This alert was generated by the same neutrino event as the previous alert(s) with the same event and run numbers.")
            same_as_ehe = 1
    w = Who()
    ...
    p = Param(name="amon_id", ucd="meta.number",dataType="int", value=str(id))
    ...
    p = Param(name="same_as_ehe", ucd="meta.number", unit=" ", dataType="int",  value=str(same_as_ehe))
    p.set_Description(["Value of 1 means this alert is based on the same neutrino as EHE alert with the same run and event numbers."])
    w.add_Param(p)
