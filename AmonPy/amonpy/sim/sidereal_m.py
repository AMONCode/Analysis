...
def coordRotate ( x, y, z ):
    ...
    # Modifcation (below) by Miles Smith, 05-20-2013
    # Old version had an argument that occassionally became 
    # problematic (acos(A) where A < -1), due to numerical precesion.
    # Rearranged terms to avoid precision error
    ...
    # new code:
    arg_new = ( sin(x) * cos(y) - sin(y) *cos(x) *cos(z) )/cos(xt)
    yt = acos(arg_new)
    
    # for testing purposes:
    #arg_old = ( sin(x) - sin(y) * sin(xt) )/( cos(y) * cos(xt) ) 
    #print arg_new - arg_old
